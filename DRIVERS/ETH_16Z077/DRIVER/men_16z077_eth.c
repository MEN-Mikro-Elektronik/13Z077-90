...
struct z77_private {
	...
	void *bdBase;
	/*!< start address of BDs in RAM (physical adr) */
	dma_addr_t bdPhys;
	u32 pollInProgress;
...
static int z77_up(struct net_device *dev);
...
static int z77_down(struct net_device *dev);
...
static void z77_set_rx_mode(struct net_device *dev)
{
	...
	Z77DBG(ETHT_MESSAGE_LVL1,
			MEN_Z77_DRV_NAME
			" z77_set_rx_mode: dev %s flags:0x%x\n",
			dev->name, dev->flags);
...
static void z77_hash_table_setup(struct net_device *dev)
{
	...
	Z77DBG( ETHT_MESSAGE_LVL3,
			MEN_Z77_DRV_NAME
			" z77_hash_table_setup: HASH0=0x%08x HASH1=0x%08x\n",
			hash0, hash1);
...
static int z77_bd_setup(struct net_device *dev)
{
	...
	for ( i = 0; i < Z077_TBD_NUM; i++ ) {
		...
		/* memVirtDma = kzalloc(Z77_ETHBUF_SIZE, GFP_KERNEL);
		memPhysDma = dma_map_single(&pcd->dev, memVirtDma, Z77_ETHBUF_SIZE,
				DMA_TO_DEVICE); */
		np->txBd[i].BdAddr = memVirtDma;
		np->txBd[i].BdPhyAddr = memPhysDma;
		Z77DBG(ETHT_MESSAGE_LVL2, 
			"--> %s TX BD %d: memVirtDma = 0x%x | memPhysDma = 0x%lx\n", 
			__func__, i, memVirtDma, (unsigned long)memPhysDma, __FUNCTION__);
		Z077_SET_TBD_ADDR(i, np->txBd[i].BdPhyAddr);
	...
	for (i = 0; i < Z077_RBD_NUM; i++ ) {
		...
		np->rxBd[i].BdPhyAddr = memPhysDma;
		Z77DBG(ETHT_MESSAGE_LVL2, 
			"--> %s RX BD %d: memVirtDma = 0x%x | memPhysDma = 0x%lx\n", 
			__func__, i, memVirtDma, (unsigned long)memPhysDma, __FUNCTION__);
		Z077_SET_RBD_ADDR(i, memPhysDma);
		smp_wmb();
...
static void z77_tx_err( struct net_device *dev)
{
	...
	for (i = Z077_TBD_NUM-1; i >= 0; i--) {
		if (Z077_GET_TBD_FLAG(i , OETH_TX_BD_DEFER | OETH_TX_BD_CARRIER
			...
			np->stats.tx_errors++;
			np->stats.collisions += Z077_GET_TBD_FLAG(i,
					OETH_TX_BD_RETRY) >> 4;
...
static int z77_init_phymode (struct net_device *dev, u8 phyAddr)
{
	...
	if ( bDoAutoneg ) {
		...
		if ( (res = mii_nway_restart(&np->mii_if )) ) {
			printk(KERN_ERR
				"*** setting autoneg. PHY mode failed\n");
			return -EINVAL;
		}
	} else {
...
static void z77_reset_task(struct work_struct *work)
{
	...
	while (test_and_set_bit(__Z77_RESETTING, &np->flags))
		msleep(1);
	...
	msleep(1);
	np->stats.tx_errors++;
	clear_bit(__Z77_RESETTING, &np->flags);
}
...
/****************************************************************************/
/** z77_up - set ethernet interface up
 *
 * \param dev			\IN net_device struct for this NIC
 *
 * \return 0 or error code
...
static int z77_send_packet(struct sk_buff *skb, struct net_device *dev)
{
	...
	Z77DBG(ETHT_MESSAGE_LVL2, "%s: %s[%d] len 0x%04x\n",
			dev->name, __func__, idxTx, skb->len );

	/* set physical addr in descriptor within the IP */
	src = (u8*)buf;
...
cont_init:
	...
	if (first_init) {
		if (z77_do_autonegotiation(dev)) {
			printk(KERN_ERR
				"*** PHY mode setting / autonegotiate failed!\n");
			return(-ENODEV);
		}
	}
...
static void z77_pass_packet( struct net_device *dev, unsigned int idx )
{
	...
	if (( np->rxBd[idx].BdAddr == NULL ) || ( idx >= Z077_RBD_NUM )) {
		printk(KERN_WARNING 
			"%s(%s): invalid index %d (max = %d) or no allocated memory for rxBd!\n",
			dev->name, __func__, idx, pkt_len, Z077_RBD_NUM);
		return;
	}

	/* synchronize buffer descriptor before read */
	prefetch(np->bdBase);
	dma_rmb();
	rxBd_status = Z077_GET_RBD_LEN( idx );
	pkt_len = Z077_GET_RBD_LEN( idx );
	if ( pkt_len > Z77_ETHBUF_SIZE || ( pkt_len == 0 )) {
		printk(KERN_ERR "%s(%s): invalid length of rxBd index %d (len=%d)\n", 
			dev->name, __func__, idx, pkt_len);
		goto err_rxBd_return;
	}

	pkt_len	-= LEN_CRC;

	skb = netdev_alloc_skb(dev, pkt_len + NET_IP_ALIGN);
	Z77DBG(ETHT_MESSAGE_LVL1, "%s: %s[%d] len 0x%04x\n",
			dev->name, __func__, idx, pkt_len );
	...
	if (skb) {
		...
		/* Synchronize rx buffer of current descriptor */
		...
		if ( idx < 32 ) {
			Z77WRITE_D32(Z077_BASE, Z077_REG_RXEMPTY0,
					1 << idx );
		}
		else {
			Z77WRITE_D32(Z077_BASE, Z077_REG_RXEMPTY1,
					1 << (idx-32));
		}
		smp_mb();
	}
...
/*******************************************************************/
/** The inverse routine to z77_up().
 *
 *  The network interface will be set down
 *
 * \param dev		\IN net_device struct for this NIC
 *
...
static int z77_down(struct net_device *dev)
{
	...
	dma_wmb();
	for (i = 0; i < Z077_TBD_NUM; i++ ) {
		memset(np->txBd[i].BdAddr, 0, Z77_ETHBUF_SIZE);
	}
	for (i = 0; i < Z077_RBD_NUM; i++ ) {
		memset(np->rxBd[i].BdAddr, 0, Z77_ETHBUF_SIZE);
	}
	Z77DBG( ETHT_MESSAGE_LVL1, "<-- %s()\n", __FUNCTION__ );
	return 0;
}
...
static int z77_close(struct net_device *dev)
{
	...
	struct pci_dev *pcd = np->pdev;
	int i = 0;
	int count = Z77_CHECK_RESET_COUNT;
	while (test_bit(__Z77_RESETTING, &np->flags) && count--)
		usleep_range(10000, 20000);
	
	WARN_ON(test_bit(__Z77_RESETTING, &np->flags));
	Z77DBG( ETHT_MESSAGE_LVL1, "--> %s()\n", __FUNCTION__ );
	np->open_time = 0;
	np->flags &= ~(IFF_UP);
	netif_stop_queue(dev);
	...
	/* free DMA resources */
	for (i = 0; i < Z077_TBD_NUM; i++) {
		/* dma_unmap_single(&pcd->dev, np->txBd[i].BdPhyAddr,
			Z77_ETHBUF_SIZE, DMA_TO_DEVICE);
		kfree(np->txBd[i].BdAddr); */
		dma_free_coherent(&pcd->dev, Z77_ETHBUF_SIZE,
				np->txBd[i].BdAddr, np->txBd[i].BdPhyAddr);
	}
	for (i = 0; i < Z077_RBD_NUM; i++) {
		/* dma_unmap_single(&pcd->dev, np->rxBd[i].BdPhyAddr,
			Z77_ETHBUF_SIZE, DMA_FROM_DEVICE);
		kfree(np->rxBd[i].BdAddr); */
		dma_free_coherent( &pcd->dev, Z77_ETHBUF_SIZE,
				np->rxBd[i].BdAddr, np->rxBd[i].BdPhyAddr);
	}
...
int men_16z077_probe( CHAMELEON_UNIT_T *chu )
{
	...
	/* get a DMAable memory region for the BDs to have
	 * the 64 Rx/Tx BD stati in sync with IP core */
	/* memVirtDma = kzalloc(PAGE_SIZE, GFP_KERNEL);
	memPhysDma = dma_map_single(&np->pdev->dev, memVirtDma, PAGE_SIZE,
			DMA_FROM_DEVICE);
	if (dma_mapping_error(&np->pdev->dev, memPhysDma)) {
		printk(KERN_ERR "%s: *** error mapping DMA space!\n" ,
			  __func__);
		goto err_free_reg;
	} */
	np->bdBase = dma_zalloc_coherent( &chu->pdev->dev,
			PAGE_SIZE, &np->bdPhys, GFP_KERNEL);
	printk( KERN_INFO MEN_Z77_DRV_NAME
		" dma_map_single BD table memory @ CPU addr 0x%p, DMA addr 0x%p\n",
		np->bdBase, np->bdPhys);

	/* Write the allocated dma memory address to IP core register */
	Z77WRITE_D32(Z077_BASE, Z077_REG_BDSTART, np->bdPhys);
	...
	/* clean BD Area */
	memset(np->bdBase, 0, Z077_BD_AREA_SIZE );
	...
	Z77WRITE_D32( dev->base_addr, Z077_REG_COREREV,
			Z77READ_D32(dev->base_addr, Z077_REG_COREREV)
...
static int men_16z077_remove( CHAMELEON_UNIT_T *chu )
{
	...
	if (!test_bit(__Z77_RESETTING, &np->flags))
		cancel_work_sync(&np->reset_task);
	dma_free_coherent(&chu->pdev->dev, PAGE_SIZE, np->bdBase, np->bdPhys);
	/* dma_unmap_single(&chu->pdev->dev, np->bdPhys,
