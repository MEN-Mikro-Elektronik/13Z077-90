...
/*
 * Count for polling __Z77_RESET condition every 10-20msec.
 */
#define Z77_CHECK_RESET_COUNT		50
...
#define Z077_REG_IPGR1		(Z077_REG_BASE+0x10) /*!< No InterPacket Gapi */
...
#define Z077_SET_TBD_ADDR(n,adr) \
	Z77WRITE_D32(Z077_BDBASE,  Z077_BD_OFFS+(((n)+TBD_OFF) * Z077_BDSIZE) + Z077_BD_OFFS_ADR, (adr))
/*!< set Tx BD Address for Tx BD nr. (n) */

#define Z077_SET_RBD_ADDR(n,adr) \
	Z77WRITE_D32(Z077_BDBASE,  Z077_BD_OFFS+( ((n)+RBD_OFF) * Z077_BDSIZE)+ Z077_BD_OFFS_ADR, adr)
/*!< set Rx BD Address for Rx BD nr. (n) */

#define Z077_GET_RBD_ADDR(n) \
	Z77READ_D32(Z077_BDBASE, Z077_BD_OFFS + ( ((n)+RBD_OFF) * Z077_BDSIZE) + Z077_BD_OFFS_ADR)
/*!< get Rx BD Address for Rx BD nr. (n) */

#define Z077_GET_TBD_ADDR(n) \
	Z77READ_D32(Z077_BDBASE, Z077_BD_OFFS + ( ((n)+TBD_OFF) * Z077_BDSIZE)+Z077_BD_OFFS_ADR)
...
#define Z077_SET_TBD_LEN(n,l) \
	Z77WRITE_D16(Z077_BDBASE, Z077_BD_OFFS + Z077_BD_OFFS_LEN+( ((n)+TBD_OFF) * Z077_BDSIZE), (l));
/*!< set ETH Frame length for Tx BD nr. (n) */

#define Z077_GET_TBD_LEN(n) \
	Z77READ_D16(Z077_BDBASE,    Z077_BD_OFFS + Z077_BD_OFFS_LEN+( ((n)+TBD_OFF) * Z077_BDSIZE))
/*!< Get ETH Frame length for Tx BD nr. (n) */

#define Z077_GET_RBD_LEN(n) \
	Z77READ_D16(Z077_BDBASE,   Z077_BD_OFFS+( ((n)+RBD_OFF) * Z077_BDSIZE) + Z077_BD_OFFS_LEN )
/*!< retrieve the length stored in Rx BD nr. (n) */

#define Z077_CLR_RBD_LEN(n) \
	Z77WRITE_D16(Z077_BDBASE,  Z077_BD_OFFS+( ((n)+RBD_OFF) * Z077_BDSIZE)+Z077_BD_OFFS_LEN, 0 );
...
#define ETHT_MESSAGE_LVL3	3 			/*!< max verbosity incl interrupts */
...
/**
 * Enumerations for current status of device
 */
/*@{*/
enum z77_state_t {
	__Z77_TESTING,	/*!< device is under test (currently unused */
	__Z77_RESETTING,	/*!< device is resetting */
	__Z77_DOWN		/*!< device is down */
};
/*@}*/
...
typedef struct z077_bd {
	...
	void *		BdAddr;		/*!< pointer to buffer (virtual addr) */
	dma_addr_t	BdPhyAddr;	/*!< pointer to buffer (physical addr) */
